...
export default function App() {
  ...
  const [selectedMarkerIndex, setSelectedMarkerIndex] = useState(null); // Index of the selected marker
  ...
  const handleMapPress = (e) => {
    ...
    // If a marker is selected, update its position
    if (selectedMarkerIndex !== null) {
      const updatedMarkers = [...markers];
      updatedMarkers[selectedMarkerIndex] = { latitude, longitude };
  ...
  const swapMarkers = () => {
    if (markers.length === 2) {
      // Swap the positions of the first and second markers
      const [firstMarker, secondMarker] = markers;
      setMarkers([secondMarker, firstMarker]);
    }
  };
      ...
      >
        ...
        {markers.map((marker, index) => (
          <Marker
            ...
            onPress={() => setSelectedMarkerIndex(index)} // Select a marker on press
          />
        ))}
        ...
        <View style={styles.buttonContainer}>
          ...
          {markers.length === 2 && (
            <TouchableOpacity
              ...
              onPress={swapMarkers} // Swap the first and second markers
            >
